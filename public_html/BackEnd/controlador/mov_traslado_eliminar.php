...
if ( $li_contador > 0 ) {
	$ls_mensaje	= 'Traslado ya se encuentra aprobado.';
}
...
if ( $li_contador == 0 ) {
	...
	$li_contador = $crud->fila_contar(DEF_TABLA_TRASLADO, $array_campo_pk, $array_valor_pk);
	if ( $li_contador > 0 ) {
		$ls_mensaje	= 'Traslado ya se encuentra rechazado.';
	}
}
...
if ( $li_contador > 0 ) {
	// Redireccionar
	if($lb_form == true) {
		header("Location: ../vista/mov_traslado_editar.php?id_codigo=".$li_codigo."&id_msgRpta=".$ls_mensaje);
	} else {
		header("Location: ../vista/mov_traslado_lista.php?id_msgRpta=".$ls_mensaje);
	}
	return;
}
...
// Invocar CRUD
$lb_result = $crud->fila_eliminar(DEF_TABLA_TRASLADO, $array_campo_pk, $array_valor_pk);
...
if($lb_result == true) {
	...
	header("Location: ../vista/mov_traslado_editar.php?id_codigo=".$li_codigo);
}
