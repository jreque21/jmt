...
if(isset($_POST) && !empty($_POST)){
	...
	$li_cuenta = $crud->fila_contar(DEF_TABLA_HORARIO, $array_campo_pk, $array_valor_pk);
	
	// Código repetido
	if ($li_cuenta > 0) {
		$ls_mensaje = 'Horario '.$ls_descripcion.' se encuentra ocupado.';
		$lb_result  = false;
	}
	...
	if ($lb_result == true) {
		...
		$array_valor	= array($ls_descripcion, $li_cod_sede, $li_cod_instructor, $li_cod_catturno, $li_cod_tarifa, $ld_fec_inicio, $ldt_hora_inicio,
								...
								$li_cod_cinturon, $li_cod_catestudiante, $li_estudiantes_max, $li_estudiantes_min, $ls_observacion, $ls_flag_estado, $_SESSION['usr_conectado'], $ldt_fecha_insercion);
		
		// Invocar inserción
		$lb_result = $crud->fila_registrar(DEF_TABLA_HORARIO, $array_campo, $array_valor, '0');
	...
	if($lb_result == true) {
		...
		header("Location: ../vista/mov_horario_nuevo.php?id_respuesta=".$ls_mensaje);
	}
