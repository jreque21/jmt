...
if(isset($_POST) && !empty($_POST)){
	...
	$li_cuenta = $crud->fila_contar(DEF_TABLA_PROMOCIONEXTPROG, $array_campo_pk, $array_valor_pk);
	
	// Código repetido
	if ($li_cuenta > 0) {
		$ls_mensaje = 'Estudiante ya se encuentra registrado para esta Promoción.';
		$lb_result  = false;
	}
	...
	if ($lb_result == true) {
		...
		// Definir estructura
		$array_campo	= array('N_COD_PROMOCION', 'N_COD_ESTUDIANTE', 'N_COD_CINTURON_ACTUAL', 'N_COD_CINTURON_NUEVO', 'D_FEC_PROG', 'D_HORA_PROG', 
								'N_COD_MONEDA', 'N_MONTO', 'N_COD_MEDIOPAGO', 'V_COMPROBANTE', 'V_FLAG_PAGO',
								'V_OBSERVACION', 'V_FLAG_PROG', 'V_AUD_USR_REG', 'D_AUD_FEC_REG');
		$array_valor	= array($li_cod_promocion, $li_cod_estudiante, $li_cod_cinturon_actual, $li_cod_cinturon_nuevo,$ld_fec_prog, $ld_hora_prog,
								$li_cod_moneda, $li_monto, $li_cod_mediopago, $ls_comprobante, $ls_flag_pago,
								$ls_observacion, $ls_flag_prog, $_SESSION['usr_conectado'], $ldt_fecha_insercion);
		
		// Invocar inserción
		$lb_result = $crud->fila_registrar(DEF_TABLA_PROMOCIONEXTPROG, $array_campo, $array_valor, '0');
	...
	// Redireccionar
	if($lb_result == true) {
		header("Location: ../vista/mov_promocionadoext_lista.php?id_codigo_padre=".$li_cod_promocion); 
	}
	else {
		header("Location: ../vista/mov_promocionadoext_nuevo.php?id_codigo_padre=".$li_cod_promocion."&id_respuesta='".$ls_mensaje."'"); 
	}
