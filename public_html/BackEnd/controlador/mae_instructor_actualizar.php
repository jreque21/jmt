...
if(isset($_POST) && !empty($_POST)){
	...
	if (!empty($ls_imagen)){
		...
		$ls_imagen_delete = $crud->fila_recuperar_campo(DEF_TABLA_INSTRUCTOR, $array_campo_pk, $array_valor_pk, 'V_FOTO');
		
		// Borrar archivo fisicamente
		if (!empty($ls_imagen_delete)) {
			
			// Preparar directorio
			$ls_imagen_delete = "../../upload/".DEF_UPLOAD_INSTRUCTOR_DIR."/".$ls_imagen_delete;
		...
		// Upload de archivo
		$ls_imagen = f_upload_archivo($_FILES["archivo"], DEF_UPLOAD_INSTRUCTOR_DIR, "C", DEF_UPLOAD_INSTRUCTOR_W, DEF_UPLOAD_INSTRUCTOR_H);
		...
		$ls_imagen = $crud->fila_recuperar_campo(DEF_TABLA_INSTRUCTOR, $array_campo_pk, $array_valor_pk, 'V_FOTO');
	...
	$array_campo_pk	= array('N_COD_INSTRUCTOR');
	$array_valor_pk	= array($li_codigo);
	$array_campo	= array('V_NOMBRES', 'V_APE_PATERNO', 'V_APE_MATERNO', 'V_FG_SEXO', 'D_FEC_NACIMIENTO', 'N_COD_TIPODOC', 'V_NRO_DOC', 'N_COD_PAIS', 'V_DIRECCION', 'V_REFERENCIA', 'V_EMAIL', 'V_FONO', 'V_MOVIL', 'N_COD_CINTURON', 'D_FEC_ALTA', 'D_FEC_BAJA', 'V_MOTIVO_BAJA', 'V_FOTO', 'V_FLAG_ESTADO', 'V_AUD_USR_MOD', 'D_AUD_FEC_MOD');
	$array_valor	= array($ls_nombres, $ls_ape_paterno, $ls_ape_materno, $ls_fg_sexo, $ld_fec_nacimiento, $li_tipo_doc, $ls_nro_doc, $li_cod_pais, $ls_direccion, $ls_referencia, $ls_email, $ls_fono, $ls_movil, $li_cod_cinturon, $ld_fec_alta, $ld_fec_baja, $ls_motivo_baja, $ls_imagen, $ls_flag_estado, $_SESSION['usr_conectado'], $ldt_fecha_actualizacion);
	
	// Invocar Actualización
	$lb_result = $crud->fila_actualizar(DEF_TABLA_INSTRUCTOR, $array_campo_pk, $array_valor_pk, $array_campo, $array_valor);
	
	// ===================================== FIN CRUD ACTUALIZAR ========================================= //
	
	// Redireccionar
	if($lb_result == true) {
		header("Location: ../vista/mae_instructor_lista.php");
	}
	else {
		header("Location: ../vista/mae_instructor_editar.php?id_codigo=".$_POST["id_codigo"]);
	}
