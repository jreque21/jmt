...
if(isset($_POST) && !empty($_POST)){
	...
	$li_cuenta = $crud->fila_contar(DEF_TABLA_ACADEMIA, $array_campo_pk, $array_valor_pk);
	
	// Código repetido
	if ($li_cuenta > 0) {
		$ls_mensaje = 'Academia '.$ls_descripcion.' se encuentra ocupado.';
		$lb_result  = false;
	}
	...
	if($lb_result == true) {
		...
		header("Location: ../vista/mae_academia_nuevo.php?id_respuesta=".$ls_mensaje);
	}
