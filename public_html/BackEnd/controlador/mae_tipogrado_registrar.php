...
if(isset($_POST) && !empty($_POST)){
	...
	$li_cuenta = $crud->fila_contar(DEF_TABLA_TIPOGRADO, $array_campo_pk, $array_valor_pk);
	
	// Código repetido
	if ($li_cuenta > 0) {
		$ls_mensaje = 'Tipo de grado '.$ls_des_larga.' se encuentra ocupado.';
		$lb_result  = false;
	}
	...
	if ($lb_result == true) {
		...
		// Invocar inserción
		$lb_result = $crud->fila_registrar(DEF_TABLA_TIPOGRADO, $array_campo, $array_valor, '0');
	...
	if($lb_result == true) {
		...
		header("Location: ../vista/mae_tipogrado_nuevo.php?id_respuesta=".$ls_mensaje);
	}
