...
if(isset($_POST) && !empty($_POST)){
	...
	$array_campo	= array('V_DESCRIPCION', 'V_LUGAR', 'V_REFERENCIA', 'D_FEC_PROG', 'N_COD_MONEDA', 'N_MONTO', 'N_COD_SEDE', 'V_OBSERVACION', 'V_FLAG_ESTADO', 'V_AUD_USR_MOD', 'D_AUD_FEC_MOD');
	$array_valor	= array($ls_descripcion, $ls_lugar, $ls_referencia, $ld_fecha, $li_cod_moneda, $ldc_monto, $li_cod_sede, $ls_observacion, $ls_flag_estado, $_SESSION['usr_conectado'], $ldt_fecha_actualizacion);
	
	// Invocar Actualización
	$lb_result = $crud->fila_actualizar(DEF_TABLA_PROMOCION, $array_campo_pk, $array_valor_pk, $array_campo, $array_valor);
	...
	if($lb_result == true) {
		...
		header("Location: ../vista/mov_promocion_editar.php?id_codigo=".$_POST["id_codigo"]);
	}
