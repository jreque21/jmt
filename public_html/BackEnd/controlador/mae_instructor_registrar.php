...
if(isset($_POST) && !empty($_POST)){
	...
	$li_cuenta = $crud->fila_contar(DEF_TABLA_INSTRUCTOR, $array_campo_pk, $array_valor_pk);
	
	// Código repetido
	if ($li_cuenta > 0) {
		$ls_mensaje = 'Instructor con Documento '.$ls_nro_doc.' se encuentra ocupado.';
		$lb_result  = false;
	}
	...
	if ($lb_result == true) {
		...
		// Definir estructura
		$array_campo	= array('V_NOMBRES', 'V_APE_PATERNO', 'V_APE_MATERNO', 'V_FG_SEXO', 'D_FEC_NACIMIENTO', 'N_COD_TIPODOC', 'V_NRO_DOC', 'N_COD_PAIS', 'V_DIRECCION', 'V_REFERENCIA', 'V_EMAIL', 'V_FONO', 'V_MOVIL', 'N_COD_CINTURON', 'D_FEC_ALTA', 'D_FEC_BAJA', 'V_MOTIVO_BAJA', 'V_FOTO', 'V_FLAG_ESTADO', 'V_AUD_USR_REG', 'D_AUD_FEC_REG');
		$array_valor	= array($ls_nombres, $ls_ape_paterno, $ls_ape_materno, $ls_fg_sexo, $ld_fec_nacimiento, $li_tipo_doc, $ls_nro_doc, $li_cod_pais, $ls_direccion, $ls_referencia, $ls_email, $ls_fono, $ls_movil, $li_cod_cinturon, $ld_fec_alta, $ld_fec_baja, $ls_motivo_baja, $ls_imagen, $ls_flag_estado, $_SESSION['usr_conectado'], $ldt_fecha_insercion);
		
		// Invocar inserción
		$lb_result = $crud->fila_registrar(DEF_TABLA_INSTRUCTOR, $array_campo, $array_valor, '0');
	...
	if($lb_result == true) {
		...
		header("Location: ../vista/mae_instructor_nuevo.php?id_respuesta=".$ls_mensaje);
	}
