...
class usuario{
	...
	{
		// Inicializar
		$_SESSION['logged_in_numdoc'] = false;
		...
		if( !empty( $data ) ){
			...
			$id_numdoc = mysqli_real_escape_string( $this->con,  $trimmed_data['id_numdoc'] );
			$id_clave = mysqli_real_escape_string( $this->con,  $trimmed_data['id_clave'] );
			
			if((!$id_numdoc) || (!$id_clave) ) {
			...
			// Sql a ejecutar
			$query = "SELECT V_COD_USER as usr_conectado, V_NOMBRES, V_EMAIL, N_COD_REFERENCIA, D_AUD_FEC_REG FROM MAE_USUARIO WHERE V_FLAG_ESTADO = '1' AND V_COD_TIPO IN ('EST_INT', 'EST_EXT') AND V_COD_USER = '$id_numdoc' AND V_CLAVE = '$id_clave' ";
			...
			if( $count == 1){
				$_SESSION = $data;
				$_SESSION['logged_in_numdoc'] = true;
				return true;
			}else{
	...
	public function f_usuario_cambioclave( array $data )
	{
		...
		if( !empty( $data ) ){
			...
			$id_clave = md5( $id_clave );
			$query = "UPDATE MAE_USUARIO SET V_CLAVE = '$id_clave' WHERE V_COD_USER = '$id_numdoc'";
