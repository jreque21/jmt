...
function f_listado($as_titulo, $as_icono, $as_msgRpta, $an_id_codigo_padre){
	...
	// Enlaces
	$url_nuevo		= "mov_promocionadoext_nuevo.php?id_codigo_padre=".$an_id_codigo_padre;
	$url_editar		= "mov_promocionadoext_editar.php?id_codigo_padre=".$an_id_codigo_padre;
	$url_lista		= "mov_promocionext_lista.php";
	$url_eliminar	= "../controlador/mov_promocionadoext_eliminar.php?id_codigo_padre=".$an_id_codigo_padre;
	...
	// Listado
	$array = $crud->fila_listar(DEF_TABLA_PROMOCIONEXTPROG, $array_campo_pk, $array_valor_pk, 'N_COD_ESTUDIANTE', 'A', 0, 1000);
	...
	<div class="content-wrapper">
		...
		<section class="content-header">
			...
			<nav aria-label="breadcrumb">
				<ol class="breadcrumb">
					<li class="breadcrumb-item active" aria-current="page"><b>Promoción Externa Nª : </b><?php echo $ls_subtitulo;?></li>
				</ol>
			</nav>
			<ol class="breadcrumb">
...
function f_formulario($as_titulo, $as_icono, $as_msgRpta, $array = "", $an_id_codigo_padre) {
	...
	if($an_id_codigo_padre) {
		...
		//$li_cod_sede	= $crud->fila_recuperar_campo(DEF_TABLA_PROMOCIONEXT, $array_campo_pk, $array_valor_pk, 'N_COD_SEDE');
		$ld_fec_prog	= $crud->fila_recuperar_campo(DEF_TABLA_PROMOCIONEXT, $array_campo_pk, $array_valor_pk, 'D_FEC_PROG');		
		$li_cod_moneda	= $crud->fila_recuperar_campo(DEF_TABLA_PROMOCIONEXT, $array_campo_pk, $array_valor_pk, 'N_COD_MONEDA');
		$li_monto		= $crud->fila_recuperar_campo(DEF_TABLA_PROMOCIONEXT, $array_campo_pk, $array_valor_pk, 'N_MONTO');
	}else{
		...
		//$li_cod_sede	= $crud->fila_recuperar_campo(DEF_TABLA_PROMOCIONEXT, $array_campo_pk, $array_valor_pk, 'N_COD_SEDE');
		// Enlaces
		$url_eliminar	= "../controlador/mov_promocionadoext_eliminar.php?id_codigo_padre=".$li_id_codigo_padre."&id_codigo=".$array['N_COD_ESTUDIANTE'];
	}

	// Enlaces
	$url_lista		= "mov_promocionadoext_lista.php?id_codigo_padre=".$li_id_codigo_padre;
	$url_registrar	= "../controlador/mov_promocionadoext_registrar.php";
	$url_actualizar	= "../controlador/mov_promocionadoext_actualizar.php";
									...
									<div class="col-sm-4">
										<div class="form-group">
											...
											<select class="form-control" id="id_cod_promocion" name="id_cod_promocion" required disabled>
												...
												$array_tipo     = $crud->fila_listar(DEF_TABLA_PROMOCIONEXT, $array_campo_pk, $array_valor_pk, 'V_DESCRIPCION', 'A', 0, 100);
...
function f_listado_ejec($as_titulo, $as_icono, $as_msgRpta){
	...
	// Enlaces
	$url_asistencia	= "mov_promocionadoextejec_editar.php";
	$url_graduado	= "mov_graduadoext_editar.php";
	$url_lista		= "mov_graduadoext_lista.php";
	...
	// Listado
	$array = $crud->fila_listar(DEF_TABLA_PROMOCIONEXT, $array_campo_pk, $array_valor_pk, 'D_FEC_PROG', 'D', 0, 100);
	...
	<script>
		$(function(){
			$('body').on('click', '#detalle input[type=checkbox]', function(event){
				...
				$.ajax({
						data:  parametros,
						url:   '../controlador/mov_promocionadoextejec_actualizar.php',
						type:  'post',
						beforeSend: function () {
	...
	<script>
		$(function(){
			$('body').on('click', '#detalle input[type=checkbox]', function(event){
				...
				$.ajax({
						data:  parametros,
						url:   '../controlador/mov_graduadoext_actualizar.php',
						type:  'post',
						beforeSend: function () {
							$('#span_rpta').text(''); // Limpieza
							console.log("Procesando, espere por favor...");
