...
$row_padre      = $crud->fila_recuperar(DEF_TABLA_PROMOCION, $array_campo_pk, $array_valor_pk);

// Almacenar Arreglo de Invitados
$array_campo_pk	= array('N_COD_PROMOCION', 'V_FLAG_PROG');
$array_valor_pk	= array($li_codigo_padre, '1');
$array_lista 	= $crud->fila_listar(DEF_TABLA_PROMOCIONPROG, $array_campo_pk, $array_valor_pk, 'N_COD_ESTUDIANTE', 'A', 0, 100);

// Datos de Sede
$array_campo_pk	= array('N_COD_SEDE');
$array_valor_pk	= array($row_padre['N_COD_SEDE']);
...
$pdf->SetTitle("Lista de Promocionados");	// Titulo

// BODY 
// =========================================================
// Titulo
$pdf->SetFont('Helvetica','B',16);
$pdf->Cell(0,0,'LISTA DE PROMOCIONADOS',0, 0,'C',false);
$pdf->Ln(6);
...
$pdf->Cell(0,0,utf8_decode($ls_desc_sede),0, 0,'L',false);
$pdf->Ln(10);
...
$pdf->Cell(60,8,utf8_decode('Cinturón Promoción'), 1, 0,'C',true);
$pdf->Cell(30,8,utf8_decode('Hora Prog.'), 1, 0,'C',true);
$pdf->Ln(8);
...
while ($row = mysqli_fetch_assoc($array_lista)) {	
	...
	$pdf->Cell(30,8, substr($row['D_HORA_PROG'],0,5), 'B', 1,'C',true);

}

// Ver reporte en pantalla
$pdf->Output('rpt_promo_promocionados.pdf','i');
