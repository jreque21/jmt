...
// Programación Cabecera
$array_campo_pk = ['N_COD_HORARIO', 'V_FLAG_ESTADO'];
$array_valor_pk = [$li_cod_horario, '1'];
...
$pdf->Cell(0,0,'LISTA DE ASISTENCIA POR HORARIO',0, 0,'C',false);
$pdf->Ln(8);
...
$pdf->Cell(75,8,'Estudiante', 1, 0,'C',true);
if ($arrayProg) {
	while ($row_dia = mysqli_fetch_assoc($arrayProg)) {	
		$pdf->Cell(16,8,utf8_decode(date_format(date_create($row_dia['D_FEC_PROG']), 'd-M')), 1, 0,'C',true);	
	}
}	
...
while ($row = mysqli_fetch_assoc($array_lista)) {	
	...
	// Filas con contenido
	$pdf->Cell(75,8, utf8_decode($row_padron['V_APE_PATERNO']).' '.utf8_decode($row_padron['V_APE_MATERNO']).' '.utf8_decode($row_padron['V_NOMBRES']), 'B', 0,'L',true);
	...
	if ($arrayProg) {
		$li_reg = 0;
		while ($row_dia = mysqli_fetch_assoc($arrayProg)) {	
			$total = mysqli_num_rows($arrayProg);
			$li_reg = $li_reg + 1;
			if ($li_reg == $total) {
				$ls_salto = 1;
			}else{
			...
			$ls_marca = $crud->f_get_asistEstxClase($li_cod_horario, $row_dia['N_CLASE'], $row['N_COD_ESTUDIANTE']);
			$pdf->Cell(16,8, utf8_decode($ls_marca), 'B', $ls_salto,'C',true);	
		}
	}
...
$pdf->Output('rpt_asistencia_x_horario.pdf','i');
