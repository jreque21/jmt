...
if ($li_cod_cinturon != '%'){
	...
	$array_valor_pk	= array($li_cod_cinturon);
	$ls_des_cinturon   	= $crud->fila_recuperar_campo(DEF_TABLA_CINTURON, $array_campo_pk, $array_valor_pk, 'V_DES_CORTA');
}else{
	$ls_des_cinturon	= 'Todos';
}

// Almacenar resultados
$array_lista = $crud->fila_rpt_estxsede($li_cod_sede, $li_cod_cinturon);
...
$pdf->SetTitle("Lista de Estudiantes");	// Titulo

// BODY 
// =========================================================
// Titulo
$pdf->SetFont('Helvetica','B',16);
$pdf->Cell(0,0,'LISTA DE ESTUDIANTES',0, 0,'C',false);
$pdf->Ln(6);
$pdf->Cell(0,0,'( INTERNA )',0, 0,'C',false);
$pdf->Ln(8);
...
$pdf->Cell(30,0,utf8_decode('Cinturon : '),0, 0,'L',false);
$pdf->Cell(0,0,utf8_decode($ls_des_cinturon),0, 0,'L',false);
$pdf->Ln(10);
...
$pdf->Cell(50,8,utf8_decode('Sede'), 1, 0,'C',true);
$pdf->Cell(54,8,utf8_decode('Cinturón Actual'), 1, 0,'C',true);
$pdf->Ln(8);
...
while ($row = mysqli_fetch_assoc($array_lista)) {	
	...
	$ls_est_sede   	= $crud->fila_recuperar_campo(DEF_TABLA_SEDE, $array_campo_pk, $array_valor_pk, 'V_NOMBRE');
	...
	$pdf->Cell(50,8, utf8_decode($ls_est_sede), 'B', 0,'C',true);
	$pdf->Cell(54,8, utf8_decode($ls_est_cinturon), 'B', 1,'C',true);

}

// Ver reporte en pantalla
$pdf->Output('rpt_estudiantes_x_sede.pdf','i');
