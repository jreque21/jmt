...
function f_listado($as_titulo, $as_icono, $as_msgRpta){
	...
	// Enlaces
	$url_nuevo		= "mae_tipogrado_nuevo.php";
	$url_editar		= "mae_tipogrado_editar.php";
	$url_lista		= "mae_tipogrado_lista.php";
	$url_eliminar	= "../controlador/mae_tipogrado_eliminar.php";
	...
	// Listado
	$array = $crud->fila_listar(DEF_TABLA_TIPOGRADO, $array_campo_pk, $array_valor_pk, 'V_DES_CORTA', 'A', 0, 100);
									...
									<table id="lista" class="table table-striped table-bordered table-hover">
										...
										<tbody>
											...
											while ($row = mysqli_fetch_assoc($array)) {
												$url_editar_fila   = $url_editar."?id_codigo=".($row["N_COD_TIPOGRADO"]);
												$url_eliminar_fila = $url_eliminar."?id_codigo=".($row["N_COD_TIPOGRADO"]);
												?>
												<tr>
													<?php

													echo "<td>";
													echo f_r_url($url_editar_fila, $row["N_COD_TIPOGRADO"],$row["V_DES_LARGA"]);
													echo "</td>";
													...
													echo "<td align='center'>";
													?>
													<a href="#deleteModal<?php echo $row["N_COD_TIPOGRADO"]; ?>" data-toggle="modal" ><i class="fa fa-trash-o"></i></a>
													<div id="deleteModal<?php echo $row["N_COD_TIPOGRADO"]; ?>" class="modal fade">
...
function f_formulario($as_titulo, $as_icono, $as_msgRpta, $array = "") {
	...
	// Enlaces
	$url_lista		= "mae_tipogrado_lista.php";
	$url_registrar	= "../controlador/mae_tipogrado_registrar.php";
	$url_actualizar	= "../controlador/mae_tipogrado_actualizar.php";
	$url_eliminar	= "../controlador/mae_tipogrado_eliminar.php";
							...
							<form  id="form_mtto" role="form" method="post" action="" enctype="multipart/form-data" autocomplete="off">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" id="id_codigo" name="id_codigo" maxlength="5" disabled
										title = "Generado por el sistema" placeholder="Codigo autogenerado" value="<?php echo $lb_edit?$array["N_COD_TIPOGRADO"]:""; ?>">
								</div>
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" id="id_des_larga" name="id_des_larga" maxlength="150" required pattern="[A-Za-zñÑáéíóúÁÉÍÓÚ\s0-9 ]{1,150}" autofocus
										title = "Letras y Números. Tamaño máximo: 150" placeholder="(*) Ejemplo : DAN" value="<?php echo $lb_edit?$array["V_DES_LARGA"]:""; ?>">	 
								</div>
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" id="id_des_corta" name="id_des_corta" maxlength="100" required pattern="[A-Za-zñÑáéíóúÁÉÍÓÚ\s0-9 ]{1,100}"
										title = "Letras y Números. Tamaño máximo: 100" placeholder="(*) Ejemplo : DAN" value="<?php echo $lb_edit?$array["V_DES_CORTA"]:""; ?>">
								</div>
								...
								<?php
								if ($lb_edit) {
									echo "<input type=hidden name=id_codigo value=\"".$array["N_COD_TIPOGRADO"]."\">";
									?>
